...
Изменения в реализации хэшей в perl v5.18.0 являются наиболее заметными
изменениями в поведении существующего кода.

По умолчанию две различные переменные хэшей с идентичными ключами и значениями
могут возвращать их содержимое в различном порядке, тогда как раньше порядок был
идентичен.

При столкновении с этими изменениями, ключём для исправления его последствий
станет принятие правила, что B<хэши - это беспорядочные коллекции> и действие в
соответствии с ним.
...
L<Carp> has been upgraded to 1.28.
